...
module.exports = function kael() {
    ...
    var NeDB = require('nedb')
        , db = new NeDB({ filename: './db/articles', autoload: true });
    ...
    function readMarkdown() {
        ...
        fs.readdir(markdownFolder, function(error, data) {
            ...
            for ( var i = 0, j = data.length; i < j; i++ ) {
                ...
                insertDocument(fileName, articlesLength);
            }
        });
    }
    ...
    // DB
    function insertDocument(fileName, articlesLength) {
        var articleTitle = fileName.replace(/-/g, ' ')
            ...
            db.find({ title: articleDocument.title }, function (error, newDoc) {
